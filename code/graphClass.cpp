...
std::vector<int> GraphClass::BFS(int vstartID, int vendID){
    ...
    do { 
        ...
        for(Edge &enext : verticeMap[vcurrent].getEdges()) {
...
std::vector<int> GraphClass::DIJKSTRA(int vstartID, int vendID){
    ...
    do { 
        ...
        if (ecurrent.getDestID() == vendID) { //compte rendu si la destination est atteinte
            ...
            return path; //renvoie le chemin final (par ID) pour l'interface graphique
        }

        closed_set.insert(ecurrent.getDestID()); 

        for (Edge &enext : verticeMap[ecurrent.getDestID()].getEdges()) {
...
std::vector<int> GraphClass::AS(int vstartID, int vendID){
